...
model = Word2Vec(
    sentences=artists_tokens + listings_tokens,
    vector_size=100,
    window=5,
    min_count=1,
    workers=4,
...
for index, artist_row in artists_df.iterrows():
    ...
    for listing_tags in listings_df["combined_tags"]:
        ...
        similarity = cosine_similarity([mean_artist_vector], [mean_listing_vector])[0][
            0
        ]
    ...
    artist_opportunities[artist_row["name"]] = opportunities_df.to_dict(
        orient="records"
    )
...
for artist_id, opportunities_df in artist_opportunities.items():
    ...
    print(
        f"{len(opportunities_df)} opportunities found for artist {artist_name} (ID: {artist_id})\n"
    )
    print("\n")
