...
class ArtJobsScraper(Scraper):
    ...
    def scrape_job_listings(
        self, base_url: str, add_fields: List[str], total_pages: int
    ) -> None:
        row_counter = 0
    ...
    def fetch_additional_info(self, link: str, add_fields: List[str]) -> List[str]:
        ...
        if response.status_code == 200:
            ...
            for t in add_fields:
                base_class = self.get_class(
                    soup, t, "taxonomy-term-reference", "field-label-inline clearfix"
                )
            ...
            try:
                desc_class = self.get_class(
                    soup, "description", "text-long", "field-label-hidden"
                )
